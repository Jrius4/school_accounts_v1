						...
						<div class="admin-pro-accordion-wrap mg-b-15 shadow-reset">
							...
							<div class="panel-group adminpro-custon-design" id="accordion">
								<div class="panel panel-default">
									<form action="includes/editmarkst1.php" method="post">
										<?php
									$rid=intval($_GET['acid']);
										...
										if($row1['ClassId']==1 || $row1['ClassId']==2){
											$sql = "SELECT tblresult.Marks,tblpapers.PaperName,tbls21.FullName,tblsubjects.SubjectName FROM tblresult join tblpapers on tblpapers.id=tblresult.PaperId join tbls21 on tbls21.RollNo=tblresult.StudentRoll join tblsubjects on tblsubjects.id=tblresult.SubjectId where tblresult.id=$rid";
										$result = mysqli_query($conn,$sql);
										$row = mysqli_fetch_assoc($result);
										}else if($row1['ClassId']==3 || $row1['ClassId']==4){
											$sql = "SELECT tblresult.Marks,tblpapers.PaperName,tbls34.FullName,tblsubjects.SubjectName FROM tblresult join tblpapers on tblpapers.id=tblresult.PaperId join tbls34 on tbls34.RollNo=tblresult.StudentRoll join tblsubjects on tblsubjects.id=tblresult.SubjectId where tblresult.id=$rid";
										$result = mysqli_query($conn,$sql);
										$row = mysqli_fetch_assoc($result);
										}else if($row1['ClassId']==5 || $row1['ClassId']==6){
											$sql = "SELECT tblresult.Marks,tblpapers.PaperName,tblastudents.FullName,tblsubjects.SubjectName FROM tblresult join tblpapers on tblpapers.id=tblresult.PaperId join tblastudents on tblastudents.RollNo=tblresult.StudentRoll join tblsubjects on tblsubjects.id=tblresult.SubjectId where tblresult.id=$rid";
										$result = mysqli_query($conn,$sql);
										$row = mysqli_fetch_assoc($result);
										}
										?>
